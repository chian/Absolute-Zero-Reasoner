...
def classify_bvbrc_step(command, stdout, stderr):
    """
    Classify a BV-BRC step as SUCCESS or FAILURE with a reason.
    Returns (status, reason).
    """
    if stderr.strip():
        return "FAILURE", f"Error: {stderr.strip()}"
    if not stdout.strip():
        return "FAILURE", "No output returned."
    return "SUCCESS", "Command executed successfully."


def compute_bio_bvbrc_reward(results, expected_output=None):
    """
    Compute reward for a multi-step bio_bvbrc action chain.
    - results: List of dicts with keys 'command', 'stdout', 'stderr'.
    - expected_output: Ground truth for the final output (if available).
    Returns (reward, stepwise_breakdown).
    """
    if not results:
        return 0.0, []
    step_penalty = 0.05
    ...
    stepwise_breakdown = []
    for r in results:
        status, reason = classify_bvbrc_step(r['command'], r['stdout'], r['stderr'])
        stepwise_breakdown.append({
            'command': r['command'],
            'status': status,
            'reason': reason,
            'stdout': r['stdout'],
    ...
    final_stdout = results[-1]['stdout'].strip()
    try:
        final_json = json.loads(final_stdout)
    except Exception:
        final_json = None
    # If ground truth is provided, compare
    if expected_output is not None and final_json is not None:
        if final_json == expected_output:
            match_score = 1.0
        elif isinstance(final_json, dict) and isinstance(expected_output, dict):
            correct = sum(1 for k, v in expected_output.items() if k in final_json and final_json[k] == v)
            match_score = correct / max(1, len(expected_output))
        else:
            match_score = 0.0
    else:
        match_score = 0.0 if expected_output is not None else (1.0 if failed_steps == 0 else 0.0)
    reward = match_score - step_penalty * failed_steps
    return max(0.0, min(1.0, reward)), stepwise_breakdown
